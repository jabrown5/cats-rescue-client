...
CatsApp.controller('DogsCtrl', function($scope, $http) {
    $scope.dogs = [];
    $scope.urlHost = 'http://localhost:9292';
    $scope.message = '';

    $scope.addDog = function(name, note) {
        // var dog = {name: name, note: note};
        // $scope.dogs.push(dog);
        // console.log($scope.dogs);
        $http({
            url: $scope.urlHost + '/dogs',
            method: 'post',
            params: { name: name, note: note }
        }).success(function(data) {
            console.log(data);
    ...
    $scope.populateList = function() {
        $http.get($scope.urlHost + '/dogs').success(function(data) {
            console.log(data);
            $scope.dogs = data;
        });
    };

    $scope.adoptDog = function(event) {
        console.log(event.dog);
        $http.delete($scope.urlHost + '/dogs/' + event.dog.id).success(function(data) {
            console.log('Your dog is adopted :)');
            $scope.message = data.message;
            $scope.populateList();
        });
    }
